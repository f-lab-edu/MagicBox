package kr.magicbox.media.adapter.in.kafka.aop;

import kr.magicbox.media.adapter.in.kafka.event.InboxEvent;
import kr.magicbox.media.adapter.in.kafka.properties.InboxProperties;
import kr.magicbox.media.adapter.out.persistence.entity.MediaInboxEntity;
import kr.magicbox.media.adapter.out.persistence.entity.MediaInboxStatus;
...
public class IdempotentAspect {
    ...
    @Around("@annotation(kr.magicbox.media.adapter.in.kafka.annotation.Idempotent)")
    public Object around(ProceedingJoinPoint pjp) {
        ...
        if (isTooOld(occurredAt)) {
            ...
            transactionTemplate.executeWithoutResult(status ->
                mediaInboxRepository.save(MediaInboxEntity.builder()
                        .key(key)
                        .topic(consumerRecord.topic())
                        .partition(consumerRecord.partition())
                        .offset(consumerRecord.offset())
                        .status(MediaInboxStatus.DEAD_LETTERED)
        ...
        return transactionTemplate.execute(status -> {
            if (mediaInboxRepository.existsByKey(key)) {
                log.warn("[Inbox] 중복 메시지 폐기. key={}", key);
                return null;
            }
            MediaInboxEntity inbox = mediaInboxRepository.save(MediaInboxEntity.builder()
                    .key(key)
                    .topic(consumerRecord.topic())
                    .partition(consumerRecord.partition())
                    .offset(consumerRecord.offset())
                    .status(MediaInboxStatus.PENDING)
                    .occurredAt(occurredAt)
                    .build());
            try {
                pjp.proceed();
