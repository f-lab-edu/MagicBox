                ...
                .flatMap(remaining -> {
                    ...
                    return admissionRedisAdapter.getBatchSize(releaseId)
                            .flatMap(savedBatchSize -> {
                                ...
                                return waitingQueuePort.getQueueSize(releaseId)
                                        .flatMap(waitingCount -> admissionQueuePort.getActiveCount(releaseId)
                                                .flatMap(activeCount -> {
                                                    int nextBatchSize = computeNextBatchSize(
                                                            currentBatchSize, waitingCount, remaining);
                                                    log.info("[SCHEDULER] AIMD 계산 releaseId={} savedBatchSize={} waitingCount={} activeCount={} nextBatchSize={}",
                                                            releaseId.value(), savedBatchSize, waitingCount, activeCount, nextBatchSize);
                                                    return admissionRedisAdapter.saveBatchSize(releaseId, nextBatchSize)
                                                            .then(vegasMetricRepositoryPort.record(releaseId, nextBatchSize, waitingCount, activeCount))
